...
parser.add_argument("--action_source", type=str, default="dds", 
                   choices=["dds", "file", "trajectory", "policy", "replay","dds_wholebody", "act"], 
                   help="Action source")
...
def main():
    ...
    try:
        ...
        # ---------------------------------------------------------
        # get robot joint names from environment
        # ---------------------------------------------------------
        robot = env.scene["robot"]

        robot_joint_names = robot.data.joint_names

        print("\n[SIM] robot joint names from env:")
        for i, name in enumerate(robot_joint_names):
            print(f"{i:02d}: {name}")

        # ---------------------------------------------------------
        # create ACT action provider
        # ---------------------------------------------------------
    ...
    print("[DEBUG] before RobotController()")

    controller = RobotController(env, control_config)

    print("[DEBUG] RobotController created")

    print("[DEBUG] before set_action_provider()")

    controller.set_action_provider(action_provider)

    print("[DEBUG] set_action_provider done")
